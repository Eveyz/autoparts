...
import { findPart, findParts, createPart, updatePart, removePart, updatePartStorage } from './part_actions'
import { createLog, updateLog, removeLog } from '../log/log_actions'
import { getToday } from '../utils'

const { ipcRenderer } = window.require("electron")

class PartListPagination extends React.Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    M.AutoInit()
    findParts({"brand": this.props.brand}, (err, data) => {
      this.setState({
        isLoading: false,
        parts: data
      })
  ...
  deletePart(id) {
    removePart(id, (err, num) => {
      ...
      this.setState({
        mode: "VIEW",
        parts: this.state.parts.filter(part => part._id !== id)
      })
    })
  }
  ...
  render() {
    const { isLoading, mode, parts, part } = this.state
...
export default PartListPagination